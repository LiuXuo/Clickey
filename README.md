...
- 托盘联动：
  ...
  - 启动行为可配置：默认启动时打开设置页；关闭后改为静默常驻托盘，该项在下次启动时生效
...
| 切换动作（默认 左键 -> 右键 -> 中键 -> 仅移动） | `hotkeys.controls.switchAction`（默认 `Enter`；可在 Settings 调整动作顺序，并单独启用/禁用“左键双击 / Ctrl+左键 / Cmd+左键 / Shift+左键”等动作） |
| 取消并退出                                      | `Esc`                                                                                                                                            |
...
- **启动行为**：默认勾选“启动时打开设置页”；关闭后改为静默启动到托盘，该项在下次启动时生效。
...
- 启动行为：`app.settingsWindow.openOnLaunch`（`true` 时启动自动打开 Settings；`false` 时静默常驻托盘）
